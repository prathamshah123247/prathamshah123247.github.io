...
const $$ = <T extends Element = Element>(selector: string) => Array.from(document.querySelectorAll<T>(selector));
...
const initPointerEffects = (): void => {
  ...
  cards.forEach((card) => {
    ...
    card.addEventListener('pointermove', (event) => {
      ...
      gsap.to(card, { rotateX: -py * 3, rotateY: px * 4, y: -4, duration: 0.25, overwrite: true });
    });
    card.addEventListener('pointerleave', () => gsap.to(card, { rotateX: 0, rotateY: 0, y: 0, duration: 0.35, overwrite: true }));
  });
};
...
const initTouchFeedback = (): void => {
  ...
  $$('.panel,.skill,.award,.cert,.project-mini,.project-main,.contact-cta').forEach((card) => {
    card.addEventListener('pointerdown', () => card.classList.add('touch-active'), { passive: true });
    ['pointerup', 'pointercancel', 'pointerleave'].forEach((type) => {
      card.addEventListener(type, () => card.classList.remove('touch-active'), { passive: true });
    });
  });
};
  ...
  const animate = (): void => {
    $$('#about .number b').forEach((element, index) => {
      ...
      gsap.fromTo(element, { textContent: 0 }, {
        ...
        onUpdate: () => { element.textContent = `${element.textContent ?? 0}${suffix}`; },
        onComplete: () => { element.textContent = `${target}${suffix}`; },
      });
    });
  };
  ScrollTrigger.create({ trigger: metricSection, start: 'clamp(top 80%)', once: true, onEnter: animate });
};
...
const initNetwork = (): void => {
  ...
  const palette = (): { a: RGB; b: RGB; c: RGB } => body.classList.contains('light')
  ...
  const draw = (now: number): void => {
    ...
    for (let k = 0; k < 3; k += 1) {
      ...
      for (let x = -40; x <= width + 40; x += 18) {
        const y = height * (0.2 + k * 0.29) + Math.sin(x * 0.006 + pulse * (1.2 + k * 0.25) + k) * 22 + Math.sin(x * 0.014 - pulse * 0.7) * 9;
  ...
  document.addEventListener('visibilitychange', () => {
    if (!document.hidden && !reduced) { last = performance.now(); requestAnimationFrame(draw); }
  });
  requestAnimationFrame(draw);
};
